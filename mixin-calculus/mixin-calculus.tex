...
and associative, dissolving the linearization problem in traditional
mixin and trait systems. We prove Turing completeness by systematically
translating the $\lambda$-calculus into MixinCalculus, demonstrating
that function application is derivable from symmetric mixin composition.
...
the RAM machine. This establishes MixinCalculus as a computational model
...
Crucially, this translation exhibits a profound \emph{asymmetry}:
...
This asymmetry demonstrates that MixinCalculus is not merely a
syntactic variant of the $\lambda$-calculus but captures fundamentally
different computational patterns.

A third insight concerns the relationship between MixinCalculus and
...
$\lambda$-calculus---is fundamentally more difficult. The denotational
semantics in Section~4 already demonstrates this complexity: defining
the meaning of MixinCalculus required a metalanguage with fixed points,
higher-order functions, and recursive domains. This sophistication is
...
\footnote{A runnable implementation using YAML syntax has been tested;
see the accompanying repository.}
...
are \emph{fundamentally different} from those of the $\lambda$-calculus,
...
fundamentally different computational primitives, not merely a
...
expressiveness stems from the properties proven in Section~4: composition
is commutative, idempotent, and associative, dissolving the linearization
problem. Other configuration languages exhibit similar patterns:
...
fields. MixinCalculus dissolves this problem (Section~4) by eliminating
...
when scalar files conflict). A proof-of-concept implementation~\cite{ratarmount2024}
demonstrates that MixinCalculus patterns emerge naturally in union mount
composition through late-binding semantics and dynamic dispatch for file
lookups across layers.
