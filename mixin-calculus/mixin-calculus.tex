...
\begin{abstract}
  ...
  The translation is adequate and fully abstract with respect to
  B\"ohm tree equivalence, yielding a fully abstract semantics
  of the lazy $\lambda$-calculus that lives entirely in naive
  set theory---powerset lattices and the Knaster--Tarski
  fixed-point theorem---without the topological infrastructure
  of domain theory.
...
\paragraph{Reference}
A reference navigates the scope hierarchy upward and then projects
properties downward. Overlay-Calculus provides two equivalent
notations for the upward navigation:
\begin{itemize}
...
\end{itemize}
Both forms retrieve the \emph{dynamic self} of the target scope---the
fully composed record after all compositions have been applied---and
...
\begin{itemize}
  \item $\qualifiedthis{\mathrm{Foo}}$ (equivalently, $\dbi{n}$ where
    $\mathrm{Foo}$ is $n$ scope levels above) retrieves the dynamic self of the
    enclosing record named $\mathrm{Foo}$.
  \item $\qualifiedthis{\mathrm{Foo}}.\ell$ (equivalently, $\dbi{n}.\ell$)
    projects property $\ell$ from
    the dynamic self of $\mathrm{Foo}$.
...
composition; one of the two qualified forms is required for
...
to the $\mathrm{argument}$ slot after all compositions.
...
In both cases the composition that performs the application is
...
\section{B\"ohm Tree Correspondence}
\label{sec:bohm-tree}

The translation $\mathcal{T}$ of Section~\ref{sec:translation}
induces a semantics on the $\lambda$-calculus via the overlay tree
equations~(\ref{eq:properties})--(\ref{eq:this}).
This section proves that the induced semantics corresponds
precisely to the B\"ohm tree~\cite{barendregt1984lambda} of
the original $\lambda$-term, establishing both adequacy and full
...
\begin{definition}[Overlay-convergence]\label{def:convergence}
  ...
  if there exists $n \ge 0$ such that the path obtained by
  following $n$ successive $\mathrm{result}$ projections from
  the root exhibits the \emph{abstraction shape}:
  \[
  ...
  Section~\ref{sec:overlay-trees}.
  We write $T{\Uparrow}$ if no such $n$ exists.
\end{definition}
...
general case of multi-path overlay composition.
...
$\ownproperties = \{\mathrm{argument}, \mathrm{result}\}$ and
no composition sources, so $\references = \varnothing$.
At the root path $p$, we have $\overlays(p) = \{p\}$ (a single
path) since there are no composition sources to introduce
additional branches.
...
and no composition sources at the outer level
($\references = \varnothing$), so $\overlays(\text{root})
= \{\text{root}\}$.
Inside the $x$ subtree, the composition
$\{\mathcal{T}(V_1),\; \mathrm{argument} \mapsto
\mathcal{T}(V_2)\}$ has exactly one composition source
...
with no composition sources at the outer level, and the
$\_\mathrm{tail}$ subtree contains exactly one composition
source.
...
$\mathcal{T}$ never produces two independent composition sources
at the same scope level that both define the same label.
In every case, application compositions are \emph{encapsulated}
inside a named property ($x$ or $\_\mathrm{tail}$), and each
such encapsulated composition has exactly one composition
source.
...
The core mechanism of the translation is that composition with
$\mathrm{argument} \mapsto \mathcal{T}(V)$ plays the role of
substitution.  We make this precise.

\begin{lemma}[Substitution]\label{lem:substitution}
  Let $M$ be an ANF term with a free variable $x$ bound by an
  enclosing $\lambda x$, and let $V$ be a closed value.
  Define the \emph{composed tree}
  ...
  (re-normalized to ANF if needed).
\end{lemma}
...
In the composed tree $C$, $\this$ resolves the reference by
walking up $n$ scope levels to the binding $\lambda$, where the composition
$\{\mathcal{T}(\lambda x.\, x),\;
\mathrm{argument} \mapsto \mathcal{T}(V)\}$ overlays the
$\mathrm{argument}$ slot with $\mathcal{T}(V)$.
By Lemma~\ref{lem:single-path}, $\this$ finds exactly one path,
...
$\mathcal{T}(M'[V/x])$.
Since $(\lambda y.\, M')[V/x] = \lambda y.\, (M'[V/x])$
(assuming $y$ is fresh), the result follows.

\paragraph{Inductive case:
$M = \mathbf{let}\; z = V_1\; V_2 \;\mathbf{in}\; M'$}
Then $\mathcal{T}(M) =
...
$\mathrm{result}$ path inside the composition
$\_\mathrm{tail} = \{\mathcal{T}(\lambda x.\, M),\;
\mathrm{argument} \mapsto \mathcal{T}(V)\}$.
This composition is exactly the composed tree $C$ of
...
\mathrm{argument} \mapsto \mathcal{T}(V_2)\}$.
This is the composed tree $C$ of
...
\mathrm{result} \mapsto \mathcal{T}(M')\}$,
whose root has
$\ownproperties = \{\mathrm{argument}, \mathrm{result}\}$.
...
$\ownproperties = \{\_\mathrm{tail}, \mathrm{result}\}$
or $\{x, \mathrm{result}\}$, and $\mathrm{argument}$ cannot
appear via $\supers$ because
$\references = \varnothing$ at the root, giving
...
\label{sec:bohm-tree-correspondence}
...
adequate but not fully abstract for PCF---it identifies
terms that observational equivalence
distinguishes~\cite{plotkin1975callbyname}.

The overlay tree semantics can thus be understood as a
computation mechanism for B\"ohm
trees~\cite{barendregt1984lambda}---the canonical fully
abstract model of the lazy $\lambda$-calculus---using only
powerset lattices and monotone operators, without the
topological infrastructure of domain theory.
In particular, this establishes that the lazy
$\lambda$-calculus admits a fully abstract denotational
semantics within naive set theory: every set-theoretic
operation involved---union, membership, powerset, monotone
fixed point---belongs to ZF without the Axiom of Choice,
and no topological or order-theoretic structure beyond
subset inclusion is needed.
