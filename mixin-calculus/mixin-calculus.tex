...
\begin{abstract}
  ...
  mixin and trait systems simply does not arise. A straightforward
  translation maps the $\lambda$-calculus into Overlay-Calculus: function
  application is just symmetric overlay composition.
...
\begin{itemize}
  ...
  \item Composition ($\sqcup$) operates at the \emph{tree level}:
    composing two overlays that share a label recursively merges their
    subtrees (union mount semantics), rather than overriding at the
    method level as in mixin and trait calculi.
    This, combined with the absence of scalars, makes $\sqcup$
  ...
  \item Self-reference resolves to \emph{multiple objects}, not one.
    ...
    either reject it (Scala) or diverge (NixOS module system).
    Since $\sqcup$ is idempotent, all objects contribute
  ...
  \item The Expression Problem~\cite{wadler1998expression} does not
    ...
    composing them makes new operations available wherever the data
    type appears, including in return types of independently
    defined operations. No design pattern is needed---this is
    simply what composition does.
...
introduced in Section~1: like an
...
These properties are immediate from the definitions above
because $\sqcup$ is a \emph{syntactic} construct, not a semantic
operator: as described in the AST paragraph, parsing eliminates
$\sqcup$ by merging $\ownproperties$ and $\references$ from both
operands via set union into a single AST node.
The operator does not appear in the semantic domain at all.
The semantic functions $\properties$, $\supers$, $\overlays$,
$\bases$, $\resolve$, and $\this$ are defined entirely over
sets---there is no binary operation in the semantics whose
operand order could matter.
Since sets are extensional---determined solely by their
elements, not by construction history---the AST is identical
regardless of the syntactic order or duplication of $\sqcup$
operands, and therefore so is every observable property
$\ell \in \properties(p)$.
...
$\{a \mapsto \{\}\} \sqcup \{b \mapsto \{\}\}$ does not reduce to
...
mutually recursive definitions unfold on demand. This observation model
...
structure.
Since Overlay-Calculus is Turing complete, the mutual recursion among
$\properties$, $\supers$, $\bases$, and $\resolve$ may not terminate
for all paths---this non-termination is the coalgebraic analogue of
divergence in the $\lambda$-calculus and is inherent to any
...
classes in $\lambda$-calculus-based systems---is just $\sqcup$.
...
symmetric, associative nature of $\sqcup$ (Section~\ref{sec:overlay-trees}) explains why such
...
to qualified this ($\ell_{\mathrm{up}}.\self$) in this paper.
