...
\DeclareMathOperator{\qualifiedthis}{this}
...
The scope contains all properties of that mixin, including those introduced by $\sqcup$.
In particular, $\{a \mapsto \{\}\} \sqcup \{b \mapsto \{\}\}$ is a single mixin whose scope
...
Given a normalized AST, what are the \emph{properties} visible at
...
With paths in hand, we can state what the normalized AST provides
at each path.

\paragraph{Normalized AST.}
An expression $e$ (Section~2) is \emph{normalized} into an AST
before evaluation by flattening all $\sqcup$ nodes.
The normalized AST provides two primitive functions at each path $p$:
\begin{itemize}
  \item $\ownmembers(p)$: the set of labels
    that have subtrees at $p$.
  \item $\references(p)$: the multiset of resolved reference pairs
    $(\ell_{\mathrm{up},*},\; \ell_{\mathrm{down},*})$,
    where each component is a label list.
\end{itemize}

During normalization, a syntactic reference
...
The suffix of $p$ from that position onward becomes
$\ell_{\mathrm{up},*} = (\ell_{\mathrm{up},1}, \ldots, \ell_{\mathrm{up},m})$.
The projections become
$\ell_{\mathrm{down},*} = (\ell_{\mathrm{down},1}, \ldots, \ell_{\mathrm{down},k})$.

Normalization populates $\ownmembers$ and $\references$ as follows:
\begin{itemize}
  ...
  \item A composition $e_1 \sqcup e_2$ takes the union of both
...
\[
  \keys(p) =
  \bigl\{\; \ell \;\big|\;
  p_{\mathrm{super}} \in \supers(p),\;
  \ell \in \ownmembers(p_{\mathrm{super}})
  \;\bigr\}
\]
...
\paragraph{Supers.}
The \emph{supers} of $p$ are the unions of every path in
the reflexive transitive closure of $\edges$:
\[
  \supers(p) =
  \bigl\{\; p_{\mathrm{union}} \;\big|\;
  p' \in \edges^*(p),\;
  p_{\mathrm{union}} \in \unions(p')
  \;\bigr\}
\]
The $\supers$ formula depends on two functions:
$\unions$ and $\edges$. We define $\unions$ first.

\paragraph{Unions.}
$\unions(p)$ is the set of paths that share
the \emph{same identity} as $p$.
When $p \neq ()$,
its unions include itself and $p_{\mathrm{init\_super}} \snoc \last(p)$ for every
super $p_{\mathrm{init\_super}}$ of $\init(p)$ that also defines $\last(p)$:
\[
  \unions(p) =
  \begin{cases}
    \{p\} & \text{if } p = ()  \\[6pt]
    \{p\}
    \;\cup\;
    \left\{\; p_{\mathrm{init\_super}} \snoc \last(p) \;\left|\;
    \begin{aligned}
      &p_{\mathrm{init\_super}} \in \supers(\init(p)), \\
      &\text{s.t.}\; \last(p) \in \ownmembers(p_{\mathrm{init\_super}})
    \end{aligned}
    \right.\right\}
    & \text{if } p \neq ()
  \end{cases}
...
\paragraph{Edges.}
The \emph{edges} of $p$ in the inheritance DAG are the paths
reachable by navigating its resolved references one step:
\[
  \edges(p) =
  \bigl\{\; p_{\mathrm{target}} \;\big|\;
  p_{\mathrm{union}} \in \unions(p),\;
  \mathrm{ref} \in \references(p_{\mathrm{union}}),\;
  p_{\mathrm{target}} \in \navigate(\init(p),\; \mathrm{ref})
  \;\bigr\}
\]
The $\edges$ formula calls $\navigate$, which we define next.

\paragraph{Navigation.}
The function $\navigate(p_{\mathrm{site}},\; \mathrm{ref})$
takes a composition-site path $p_{\mathrm{site}}$
and a resolved base
$\mathrm{ref} = (\ell_{\mathrm{up},*},\; \ell_{\mathrm{down},*})$,
and returns a set of target paths, as follows.
Since $\ell_{\mathrm{up},*}$ is a suffix of the definition-site path,
it is also a valid absolute path $p_{\mathrm{def}}$.

\begin{enumerate}
  \item Start with the set
    $\mathit{current\_set} = \{p_{\mathrm{site}}\}$
    and $p_{\mathrm{def}} = \ell_{\mathrm{up},*}$.
  \item While $p_{\mathrm{def}}$ is non-empty:
    \[
      \mathit{current\_set} \;\leftarrow\;
      \bigl\{\; p' \;\big|\;
      p_{\mathrm{current}} \in \mathit{current\_set},\;
      p' \in
      \qualifiedthis(p_{\mathrm{current}},\;
      p_{\mathrm{def}})
      \;\bigr\}
      \qquad\qquad
      p_{\mathrm{def}} \;\leftarrow\;
      \init(p_{\mathrm{def}})
    \]
    At each step, $\qualifiedthis$ maps the definition-site path
    to the corresponding composition-site paths,
    then $\init$ shortens the path by one label.
  \item For each $p_{\mathrm{current}} \in \mathit{current\_set}$,
    append $\ell_{\mathrm{down},*}$:
    $p_{\mathrm{current}} \snoc \ell_{\mathrm{down},1} \snoc \ell_{\mathrm{down},2} \cdots \snoc \ell_{\mathrm{down},k}$.
\end{enumerate}
The result is the set of target paths reached by all routes.
...
The navigation loop calls $\qualifiedthis$, which we now define.

\paragraph{Qualified this.}
$\qualifiedthis(p,\; p_{\mathrm{def}})$ returns,
for each definition-site path $p_{\mathrm{def}}$,
the set of $\init(p_{\mathrm{super}})$ values
where $p_{\mathrm{super}}$ matches $p_{\mathrm{def}}$
among the supers of $p$:
\begin{align*}
  \qualifiedthis(p,\;
  p_{\mathrm{def}}) = \left\{\;
  \init(p_{\mathrm{super}}) \;\left|\;
  \begin{aligned}
    &p_{\mathrm{super}} \in \supers(p), \\
    &\text{s.t.}\; p_{\mathrm{super}} = p_{\mathrm{def}}
  \end{aligned}
  \right.\right\}
\end{align*}
Intuitively, $\qualifiedthis(p,\;
p_{\mathrm{def}})$ answers:
``in the fully composed path $p$, at which
composition-site paths does the definition-site path
$p_{\mathrm{def}}$ appear as a super?''

Note that $\qualifiedthis$ returns a \emph{set} of paths, not a single path.
...
symmetric, associative nature of $\sqcup$ (Section~4) explains why such
