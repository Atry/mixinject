...
not macro-expressible under the equivalence notions fixed in
Section~\ref{sec:asymmetry} and Appendix~\ref{app:expressiveness}---
inheritance-calculus enables open extension that changes
operational equivalence on $\lambda$-terms, establishing a
strict separation in Felleisen's sense of
expressiveness~\cite{felleisen1991expressive}
...
\begin{itemize}
  \item We present inheritance-calculus, a minimal computational model for
    declarative programming that contains only three constructs (record,
  ...
  \item Inheritance operates at the \emph{tree level}:
    ...
    inheritance sources, so inheriting from two mixins that share a label
    recursively merges their subtrees (deep merge), rather
    than overriding at the method level as in prior mixin and trait calculi.
  ...
  \item Self-referential records require no fixed-point combinator.
    The $\lambda$-calculus translation maps each application to an
    inheritance; the resulting inheritance tree \emph{is} the call stack.
    The $\this$ function of Section~\ref{sec:mixin-trees} resolves
  ...
  \item Self-reference resolves to \emph{multiple objects}, not one.
    ...
    equally; this semantics does not introduce a route-priority
    disambiguation rule.
  ...
  \item We observe further properties: extensibility in the sense of the
    ...
    (Section~\ref{sec:practical-function-color}).
  \item Open extension via inheritance changes the operational
    equivalence of $\lambda$-terms, establishing via
    Felleisen's Theorem~3.14~\cite{felleisen1991expressive}
    that the $\lambda$-calculus cannot macro-express inheritance-calculus
    (Section~\ref{sec:asymmetry}).
\end{itemize}
...
A mixin tree has a single syntactic construct---the mixin---that
simultaneously plays the roles of field, method, class, and object
in object-oriented languages.
A mixin that defines no children acts as a field or method;
a mixin that defines children acts as a class;
and every path in the tree is already an object, since its
observable behavior is determined by the semantic functions below.
This unification is the source of the calculus's economy:
the six equations that follow suffice because they need only
describe one kind of entity.

Unlike the $\lambda$-calculus, which requires reduction rules
($\beta$-reduction) to specify how terms compute, inheritance-calculus has
...
$\supers$, $\overrides$, $\bases$, $\resolve$, and $\this$, constitute the
\emph{entire} semantics. This is the observational perspective
introduced in Section~\ref{sec:introduction}: like an
F-coalgebra~\cite{rutten2000universal}, a mixin tree's behavior is
determined by observation (property presence), not by reduction.

Each function has an analogue in object-oriented languages:
...
$\bases$ collects the direct base classes,
and $\overrides$ generalizes method override to the tree setting,
collecting the identity-sharing paths that arise
...
\paragraph{Overrides}
Because inheritance-calculus has no separate notion of ``method,''
a mixin simultaneously plays the role of a class and the role
of a method in object-oriented languages.
When a mixin is viewed as a method, a child scope should be
...
inheritance-calculus. There are no reduction rules, no evaluation contexts,
...
order reductions occur. In inheritance-calculus, an inheritance tree such as
...
The detailed well-definedness argument, which identifies the lattice,
verifies positivity of each equation, and handles the transitive
...
functions of Section~\ref{sec:mixin-trees} induces a semantics
...
of the translated mixin tree corresponds exactly to the
B\"ohm tree~\cite{barendregt1984lambda} of the original term:
the translation is adequate, meaning a term converges iff it has a head
normal form, and fully abstract with respect to
$\lambda$-definable contexts, meaning two $\lambda$-terms are
equivalent under the translation iff they have the same B\"ohm
tree.
...
\emph{the lazy $\lambda$-calculus admits a fully abstract
  semantics (w.r.t.\ $\lambda$-definable contexts)
that lives entirely in naive set theory.}
...
The mixin tree semantics achieves full abstraction with
...
the translation $\mathcal{T}$ is a fully faithful embedding of the
...
(Theorem~\ref{thm:cartesian-non-expressibility}).
...
strictly more expressive than the $\lambda$-calculus:
the forward embedding is macro-expressible
(Theorem~\ref{thm:forward-macro}), but the reverse is not
(Theorem~\ref{thm:cartesian-non-expressibility}).
...
Several features of inheritance-calculus are primitive yet have no
direct counterpart in the $\lambda$-calculus.  Encoding any one
of them requires pervasive, non-local changes to program
structure.

\begin{description}
  \item[Self-referential records with lazy evaluation]
    A mixin's properties can reference other properties and enclosing scopes
    ...
    references~\cite{boudol2004recursive} (e.g., Haskell-style
    thunks or ML-style \texttt{ref} cells), obscuring the
    declarative intent.

  \item[Open recursion with symmetric inheritance]
    Inheritance merges definitions from independent sources,
    all sharing the same inherited mixin tree.  Encoding this in
  ...
  \item[Commutative, idempotent merge]
    Inheritance in inheritance-calculus is symmetric:
    ...
    these properties requires additional
    machinery~\cite{harper1991record,cardelli1992extensible}.

  \item[Relational semantics]
    Every inheritance-calculus operation is defined by recursion on
    mixin trees, not on individual values.
    When a mixin tree is viewed as a trie, the correspondence
    with relational algebra becomes precise:
    a path from the root to a leaf encodes a single value
    (e.g., $\mathrm{Successor} \to \mathrm{predecessor} \to
    \mathrm{Zero}$ encodes the natural number~$1$);
    a trie union encodes a \emph{set} of such values, i.e., a
    unary relation.
    An operation that takes two trie-union operands traverses
    every combination of their paths through $\bases$
    (equation~\ref{eq:bases}), which is a cross-join;
    the transitive closure $\bases^*$ in $\supers$
    (equation~\ref{eq:supers}) computes the union of all
    reachable paths; and $\properties$
    (equation~\ref{eq:properties}) projects the observable labels.
    ...
    In the Case Study (Section~\ref{sec:case-study}), the
    $\mathrm{Plus}$ and $\mathrm{Equal}$ operations are defined
    on single Nat values.  When applied to set-valued mixins
    $\{1,2\}$ and $\{3,4\}$, the same definitions, without any
    modification, produce the set of all pairwise sums
    $\{4,5,6\}$.  This is relational
    semantics in action: each operation distributes over every
    combination of its inputs.  The behavior is native: the
    semantic equations of Section~\ref{sec:mixin-trees}
    resolve every property to a set of paths, so single-valued
    and set-valued computation are the same mechanism.  No code
    change is needed to move from one to the other.

  \item[Function color blindness]
    \label{sec:practical-function-color}
    In a web
    application\selfcite{~\cite{mixin2025}}{~(supplementary material)}
    initially targeting Python's synchronous \texttt{sqlite3} and
    \texttt{http.server} modules,
    the business logic MIXINv2 file contained no async-specific code.
    When the FFI layer was replaced with one built on
    \texttt{aiosqlite} and \texttt{starlette}, the business logic
    MIXINv2 file did not change.
    This is an instance of function color
    blindness~\cite{nystrom2015color}: async and sync functions are
    distinct types in the host language, yet the MIXINv2 file is
    ...
    under a CPS interpretation, or with direct implementations
    under a direct-style interpretation, without explicit monad machinery.

  \item[Immunity to nonextensibility]
    Programming languages typically provide a mix of extensible and
    nonextensible constructs.
    In such languages, extensibility is opt-in: frameworks
    must encode nonextensible constructs into extensible constructs
    using sophisticated
    ...
    In contrast, every construct of inheritance-calculus
    (Section~\ref{sec:syntax}) is extensible
    via deep merge (equation~\ref{eq:overrides}).
    There is no nonextensible construct, so it is
    impossible to write a nonextensible program.
    In the Case Study (Section~\ref{sec:case-study}),
    new operations and data types are added without
    modifying existing files.
    Tree-level inheritance
    (Section~\ref{sec:discussion-expression-problem}) and
    multi-path qualified this (equation~\ref{eq:this};
    Appendix~\ref{app:scala-multi-path}) account for this.
    The Expressive Asymmetry
    (Section~\ref{sec:asymmetry};
    Theorem~\ref{thm:cartesian-non-expressibility}) provides the
    formal counterpart: the mixin trees in its separating
    construction are mechanical translations of
    $\lambda$-terms via $\mathcal{T}$
    (Section~\ref{sec:forward-translation}), yet a new observable
    projection is added to them without modification.
    This is the extensibility that the Expression
    Problem~\cite{wadler1998expression} demands.%
...
\noindent
Each of these features is primitive in inheritance-calculus.
In the $\lambda$-calculus, encoding them requires
pervasive changes to program structure:
every scope must thread a mutable store for lazy allocation,
every method invocation must consult a dynamically constructed
dictionary for open recursion, and every composition must
be wrapped in order-insensitive infrastructure for commutativity.
These are global, cascading transformations, not local
rewrites of individual terms.
In common sense, inheritance-calculus is strictly more expressive
...
to nonextensibility.
This section shows that immunity determines each semantic choice:
every alternative design is vulnerable to nonextensibility
in a specific way.

\begin{center}
  \begin{tabular}{lll}
    \textbf{Variant} & \textbf{Vulnerability}
    & \textbf{Representative systems} \\
    \hline
    Functions as unit of reuse
    & same-label extensions conflict
    & trait systems~\cite{scharli2003traits}, OOP languages \\
    Early binding
    & mixin cannot replace function
    & CUE~\cite{cue2019} \\
    Dynamic scope
    & mixin cannot replace function
    & Jsonnet~\cite{jsonnet}, union file systems \\
    Single-target $\this$
...
\begin{description}
  \item[Functions as unit of reuse]
    When the composition mechanism is function application,
    the reuse unit is the closure.
    A closure's internal structure is not visible to the
    composition system: two closures that define the same label
    cannot be merged.
    Systems must reject the conflict
    (trait systems~\cite{scharli2003traits} flag same-name methods),
    linearize (MRO~\cite{c3linearization}), or shadow.
    Extending an entity with a new operation requires modifying
    the original definition or encoding extensibility via separate
    machinery (visitor pattern,
    object algebras~\cite{oliveira2012extensibility},
    finally tagless interpreters~\cite{carette2009finally}).
    In inheritance-calculus, the mixin is the unit of reuse.
    A mixin's labels are visible to the composition system, and
    deep merge (equation~\ref{eq:overrides}) combines same-label
    definitions from independent sources.
    This choice is what makes the framework of
    Section~\ref{sec:mixin-trees} possible:
    the six equations
    (\ref{eq:properties})--(\ref{eq:this}) are set-theoretic
    functions over a single meta-language object---the $\Path$ that
    identifies a mixin---
    with no function spaces, no reduction rules,
    and no substitution
    (Section~\ref{sec:forward-translation}).
    A function-based semantics requires function spaces
    (Scott domains satisfying $D \cong [D \to D]$),
    reduction rules ($\beta$-reduction), and
    capture-avoiding substitution---machinery that the
    mixin-based framework avoids entirely.
    Functions may exist at the periphery
    (FFI, host-language interop);
    core composition does not depend on closures, so their opacity
    does not affect extensibility.
    The $\lambda$-calculus embedding
    (Section~\ref{sec:forward-translation}) translates
    $\lambda$-functions into mixin records
    $\{\mathrm{argument} \mapsto \{\},\;
    \mathrm{result} \mapsto \ldots\}$,
    retaining their computational semantics without their opacity.
  ...
  \item[Dynamic scope]
    ...
    Dynamic scope does not preserve de~Bruijn index semantics:
    the $\lambda$-calculus requires variable references to resolve
    at the lexical definition site.
    In the equations, $\resolve$~(\ref{eq:resolve}) starts from
    the definition-site path $\init(p_{\mathrm{def}})$, and
    $\this$~(\ref{eq:this}) uses the de~Bruijn index~$n$
    determined at the definition site;
    dynamic scope would replace both with a runtime context,
    losing the lexical structure that the $\lambda$-calculus
    embedding relies on.
    When the multi-path example
    (Appendix~\ref{app:scala-multi-path}) is translated to
    Jsonnet, \texttt{outer} resolves to the merged
    \texttt{MyObjectA} rather than to \texttt{Object1} or
    \texttt{Object2} separately, collapsing the two inheritance
    routes into one.
  ...
  \item[Single-target $\this$]
    ...
    In the equations, $\this$~(\ref{eq:this}) tracks a frontier
    \emph{set}~$S$ rather than a single path;
    $\resolve$~(\ref{eq:resolve}) inherits the multi-valued
    result, and $\bases$~(\ref{eq:bases}) collects targets from
    all resolved paths.
    Restricting $S$ to a single element would require a
    disambiguation rule absent from the equations.
    Functions-as-reuse-unit and single-target $\this$ affect
    extensibility at different levels: opaque functions prevent
    same-level merging; single-target $\this$ prevents cross-level
    (nested) extensions from coexisting.
...
\noindent
Inheritance-calculus is the design point where immunity to
nonextensibility holds.
The mechanisms present in the alternative designs---MRO,
framework encodings, explicit wiring, runtime context threading,
duplicate-declaration errors---are absent from the equations of
Section~\ref{sec:mixin-trees}.
...
Graph models are sensible (they equate all unsolvable
terms) but not fully abstract:
Bucciarelli and Salibra~\cite{bucciarelli2008graph}
proved that the greatest sensible graph theory equals the
B\"ohm tree theory~$\mathcal{B}$, which is
strictly coarser than observational equivalence for the lazy
$\lambda$-calculus.
...
same B\"ohm tree.
Filter models are more discriminating than graph models but
are still not fully abstract for the lazy $\lambda$-calculus,
and require the additional machinery of intersection type
theory and algebraic lattices.
...
The multiset structure corresponds to non-idempotent
...
For the lazy $\lambda$-calculus, full abstraction has been
achieved only through game
semantics~\cite{abramsky1993lazy,abramsky1995games};
none of the three set-theoretic families attains it.
...
Moreover, all three identify terms more coarsely than
observational equivalence---graph and filter models coincide
with B\"ohm tree equality, while relational models cannot
even host a standard reflexive
object~\cite{ehrhard2005finiteness}---and none provides a
fully abstract semantics for the lazy $\lambda$-calculus
without resorting to game-theoretic machinery.
...
coalgebraic semantics of Section~\ref{sec:mixin-trees}.
...
behavior. These systems implement inheritance-calculus patterns ad hoc.
...
of inheritance-calculus upon us.
...
\begin{theorem}[Non-expressibility of inheritance-calculus]
\label{thm:cartesian-non-expressibility}
There is no macro-expressible translation from
inheritance-calculus to the $\lambda$-calculus.
\end{theorem}
...
By the B\"ohm Tree Correspondence
(Theorems~\ref{thm:adequacy} and~\ref{thm:full-abstraction}),
$\mathscr{L}_1$ conservatively extends $\mathscr{L}_0$:
for closed $\lambda$-terms $M$,
$\mathcal{T}(M){\Downarrow}$ if and only if $M$ has a
head normal form.
...
\begin{align*}
  \mathrm{eq} &\mapsto \{\mathrm{argument} \mapsto \{\},\;
    \mathrm{result} \mapsto \{
      ...
      & \quad
      \_\mathrm{tail} \mapsto \{x_3.\mathrm{result},\;
...
(Section~\ref{sec:forward-translation}).
Since both $\mathrm{ANF}$ and $\mathcal{T}$ are
macro-expressible, their composition is too,
so $e_1, e_2 \in \mathscr{L}_0$.
...
$e_1.\mathrm{repr}.\mathrm{firstOperand} = \mathrm{false}$
and
$e_2.\mathrm{repr}.\mathrm{firstOperand} = \mathrm{true}$.
...
Inheritance-calculus is strictly more expressive than the
$\lambda$-calculus in the sense of
...
$\supers(\mathrm{MyObjectA})$ for override paths matching
...
Section~\ref{sec:mixin-trees} is well-defined.
...
& &\text{(sets of site--override pairs)} \\
D_{\overrides} &= \mathcal{P} \to \mathcal{P}(\mathcal{P})
& &\text{(sets of paths)} \\
...
The product
$D = D_{\properties} \times D_{\supers} \times D_{\overrides}
...
$(\properties, \supers, \overrides, \bases, \resolve, \this)$
...
$\supers(p) =
\bigl\{\, (\init(p_{\mathrm{base}}), p_{\mathrm{override}}) \bigm|
  p_{\mathrm{base}} \in \bases^*(p),\;
  p_{\mathrm{override}} \in \overrides(p_{\mathrm{base}})
\,\bigr\}$.
Both $\bases^*$ and $\overrides$ appear as sets being iterated
over.
As noted above, $\bases^*$ is monotone in $\bases$.
Enlarging either $\bases$ or $\overrides$ can only add elements to the
result.
\emph{Positive in $\bases$ and $\overrides$.}

\paragraph{Equation~(\ref{eq:overrides}): $\overrides$}
For $p \neq ()$:
\begin{multline*}
  \overrides(p) = \{p\} \cup
...
\begin{multline*}
  ...
  \,\bigr\}.
\end{multline*}
Both $\overrides$ and $\resolve$ appear as sets being iterated
over.
The function $\inherits$ is an AST primitive.
Enlarging either $\overrides$ or $\resolve$ can only add targets to
the result.
\emph{Positive in $\overrides$ and $\resolve$.}
...
$S' = \bigl\{\, p_{\mathrm{site}} \bigm|
  p_{\mathrm{current}} \in S,\;
  (p_{\mathrm{site}}, p_{\mathrm{override}}) \in
  \supers(p_{\mathrm{current}}),\;
  p_{\mathrm{override}} = p_{\mathrm{def}}
\,\bigr\}$.
The function $\supers$ appears as the set being iterated over.
The filtering condition $p_{\mathrm{override}} = p_{\mathrm{def}}$
