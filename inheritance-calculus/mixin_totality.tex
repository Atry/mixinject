...
\title{MIXINv2: A Total Language with Lazy Evaluation and Automatic Termination}
\author{}
\date{}
...
\begin{abstract}
We present MIXINv2, a programming language that achieves a unique combination of properties: compile-time Turing completeness, runtime totality (weak totality/productivity), support for circular references, and lazy evaluation. Unlike existing total languages that require manual termination proofs, MIXINv2 uses a simple naming convention combined with compile-time checks to automatically guarantee productivity. This makes MIXINv2 the first total language accessible to programmers without formal verification expertise.
\end{abstract}
...
\textbf{MIXINv2 guarantees weak totality:}
\begin{itemize}
...
\subsection{MIXINv2's Innovation}

MIXINv2 achieves totality \textbf{automatically} through a simple naming convention:
\begin{itemize}
...
This naming convention, combined with MIXINv2's tree-based symbol structure, guarantees totality without any manual proofs.

\section{Key Concepts}

\subsection{Symbol Tree}

Every MIXINv2 program's symbols form a \textbf{tree structure}:
...
\begin{itemize}
  ...
  \item MIXINv2 prohibits referencing the root from within nested scopes
...
In MIXINv2, lazy evaluation ensures that accessing any finite depth terminates, even when the conceptual structure is infinite.

\subsection{Instantiation Levels}

To ensure weak totality, MIXINv2 uses a concept similar to \textbf{universe levels} in type theory.
...
MIXINv2 uses lazy evaluation instead of explicit guardedness, achieving the same totality guarantee with simpler syntax.

\section{MIXINv2's Four Properties}
...
\begin{itemize}
  ...
  \item MIXINv2 uses \textbf{lazy evaluation + automatic guards}
\end{itemize}
...
\textbf{MIXINv2 verifies this automatically} by checking that:
\begin{itemize}
...
\hline
\textbf{MIXINv2} & \checkmark & \checkmark & \checkmark & \checkmark \\
\hline
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Aspect} & \textbf{Idris/Agda/Coq} & \textbf{MIXINv2} \\
\hline
...
\section{Why MIXINv2's Combination is Unique}
...
\begin{itemize}
  ...
  \item \textbf{MIXINv2}: \textbf{Total + Lazy + Automatic}
\end{itemize}

\subsection{The Secret: Tree Structure + Naming Convention + Level System}

MIXINv2 achieves automatic totality through:
...
MIXINv2 achieves a unique combination of properties:
\begin{enumerate}
...
MIXINv2 is the \textbf{first total language accessible to all programmers}:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Practical applications}: Verified software development using MIXINv2's automatic totality
\end{itemize}

\section*{Acknowledgments}

This work builds on decades of research in dependent types, lazy evaluation, and termination analysis. We acknowledge the foundational work in Haskell, Agda, Coq, and Idris that inspired MIXINv2's design.
