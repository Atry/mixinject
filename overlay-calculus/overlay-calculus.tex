...
\begin{abstract}
  Just as the $\lambda$-calculus uses three primitives (abstraction, application, variable) as the foundation of functional programming, overlay-calculus uses three primitives (record, definition, inheritance) as the foundation of declarative programming. It trivially embeds the $\lambda$-calculus, although the entire semantics rests solely on naive set theory; as a consequence, all constructs including inheritance are inherently commutative, idempotent, and associative; the linearization problem of multiple inheritance does not arise. This induces a fully abstract semantics of the lazy $\lambda$-calculus with respect to B\"ohm tree equivalence~\cite{barendregt1984lambda}. Overlay-calculus is distilled from the Overlay language, a practical implementation in which we observed further emergent phenomena: the same code acts as different function colors~\cite{nystrom2015color}; ordinary arithmetic yields the relational semantics of logic programming~\cite{vanemden1976semantics}; self-reference resolves to multiple targets; and programs are immune to the Expression Problem~\cite{wadler1998expression}. This makes overlay-calculus strictly more expressive than the $\lambda$-calculus in both common sense and Felleisen's sense~\cite{felleisen1991expressive}. These properties suggest applications to configuration languages, dependency injection, object-oriented programming, composable effect systems, modular software architectures, file-system-as-compiler, general-purpose programming, and no-code development.
\end{abstract}
...
$\lambda$-calculus into overlay-calculus requires five local,
...
overlay enables open extension that changes
...
\begin{itemize}
  ...
  \item We observe further properties: extensibility in the sense of the
    ...
    (Section~\ref{sec:practical-function-color}).
  \item Open extension via overlay changes the operational
    equivalence of $\lambda$-terms, establishing via
    Felleisen's Theorem~3.14~\cite{felleisen1991expressive}
    that the $\lambda$-calculus cannot macro-express overlay
    (Section~\ref{sec:asymmetry}).
\end{itemize}
...
contexts only; overlay contexts are strictly more
...
lazy $\lambda$-calculus into overlay-calculus
...
overlay-contextual equivalence is clarified by the expressive
asymmetry of the next section:
overlay contexts can separate $\lambda$-terms that are
B\"ohm-tree equivalent, so
${\approx_\lambda} \subsetneq {(\approx_{\mathrm{ov}}|_{\mathrm{im}(\mathcal{T})})}$.
This gap is precisely the source of overlay-calculus's
...
separated in overlay-calculus by overlaying new observable
projections onto an existing definition via open extension.
By Felleisen's expressiveness
criterion~\cite{felleisen1991expressive}, overlay-calculus is
...
The underlying mechanism is open extension: overlay
...
Several features of overlay-calculus are primitive yet have no
...
\begin{description}
  ...
  \item[Relational semantics]
    Every overlay-calculus operation is defined by recursion on
    overlay trees, not on individual values.
    When an overlay tree is viewed as a trie, the correspondence
    ...
    on single Nat values.  When applied to set-valued overlays
    ...
    semantic equations of Section~\ref{sec:overlay-trees}
  ...
  \item[Immunity to nonextensibility]
    ...
    In contrast, every construct of overlay-calculus
    (Section~\ref{sec:syntax}) is extensible
    via deep merge (equation~\ref{eq:overlays}).
    ...
    formal counterpart: the overlay trees in its separating
    ...
    \footnote{As an untyped calculus, overlay-calculus cannot violate
      ...
      overlay-calculus\selfcite{~\cite{mixin2025}}{~(supplementary
...
\noindent
Each of these features is primitive in overlay-calculus.
...
In common sense, overlay-calculus is strictly more expressive
...
\begin{description}
  ...
  \item[Multi-target qualified this]
...
between overlay-calculus and the $\lambda$-calculus.
...
embedding of the $\lambda$-calculus into overlay-calculus.
\end{theorem}

\begin{proof}
Each of the five rules of $\mathcal{T}$ replaces one
$\lambda$-calculus construct with an overlay-calculus expression
that depends only on the construct's immediate subexpressions.
No rule inspects the surrounding context.
\end{proof}

\begin{theorem}[Non-expressibility of overlay]
\label{thm:cartesian-non-expressibility}
There is no macro-expressible translation from
overlay-calculus to the $\lambda$-calculus.
\end{theorem}

\begin{proof}
Let $\mathscr{L}_1$ be the overlay-calculus and let
$\mathscr{L}_0$ be the sublanguage consisting of overlay
...
$\mathrm{BT}(M) = \mathrm{BT}(N)$.
Define $\cong_1$ on $\mathscr{L}_1$ as overlay-contextual
equivalence:
$S \cong_1 S'$ iff for every overlay context $C[\cdot]$,
...
In overlay-calculus, applying a function record to an argument
is: inherit the function, override its $\mathrm{argument}$
slot, and project $\mathrm{result}$.
Define overlay trees $e_1$ and $e_2$ that apply
...
Now define an overlay $C$ on $S$ that extends $\mathrm{eq}$
with a $\mathrm{repr}$ scope.
The overlay targets the inner $\lambda b$ scope (the
...
These produce structurally distinct overlay trees
...
$\mathscr{L}_0$ cannot macro-express the overlay facilities of $\mathscr{L}_1$.
\end{proof}

\begin{corollary}
\label{cor:expressive-asymmetry}
Overlay-calculus is strictly more expressive than the
...
over $\lambda$-calculus contexts, not over all overlay contexts.
