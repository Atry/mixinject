...
Through iterative refinement of these primitives---beginning with
...
In developing real programs in the Overlay language---boolean logic and
...
that operations on tries automatically become Cartesian products---for
example, adding $\{1,2\}$ to $\{3,4\}$ yields $\{4,5,6\}$---which
...
\end{align*}
$\mathrm{Result}.\mathrm{sum}$ evaluates to the trie $\{4,5,6\}$---all pairwise sums.
...
that both extend an entity with the same name---the situation
...
structurally legitimate cases---adding complexity, not
removing it.
...
These mechanisms---early-binding freezing, runtime context
threading, MRO, shadowing, route selection, error
...
\emph{removing} structure from an existing model---fewer
abstractions, worse ergonomics, no new expressiveness.
...
not by linearization or method-level override---the same overlay
