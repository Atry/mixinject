...
\subsection{From \texorpdfstring{$\lambda$}{λ}-Calculus to Overlay-calculus}
...
normal form, and fully abstract, meaning two terms are
...
single values produce, without modification, the relational
semantics of logic programming (Datalog, Prolog):
...
\begin{enumerate}
  ...
  \item \textbf{Relational semantics.}
    ...
    overlay trees, not on individual values.  When an overlay
    tree is viewed as a trie, every path from the root to a leaf
    is a tuple; tree-level recursion then becomes join,
    projection, and union over those tuples---the
    relational algebra of Datalog.  The carrier is a trie rather
    than a flat relation, and the fixed point is coinductive
    (greatest) rather than Datalog's inductive (least), but the
    relational structure of the computation is the same.
